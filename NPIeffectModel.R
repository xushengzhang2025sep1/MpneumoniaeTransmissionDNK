##-------------------------------------------------------------------------------------------------------##
#######################  Transmission dynamics due to NPIs ################################################

NPIeffectModel <- function (times, births, beta, alpha, IP, S0, I0, nsim, stochastic, controlStart,  controlEnd, betachangeNPI, tEnd) 
{
  ...
  for (n in 1:nsim) {
    ...
    for (t in 2:length(times)) {
      ...
      if(t > controlStart & t <= tEnd){ lambda <- min(S[t - 1], unname(betachangeNPI[t-floor(controlStart)]*beta[t - 1] * S[t - 1] * (I[t - 1])^alpha))}
      if (stochastic) {
